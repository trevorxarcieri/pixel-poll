...
def main() -> None:
    ...
    loop.create_task(put_commands())
    loop.run_forever()


async def put_commands() -> None:
    """Put some commands in the queue for testing."""
    queue.put_sync(VoteCommand.START)
    await uasyncio.sleep(1)  # wait for 1 second
    queue.put_sync(VoteCommand.INDICATE_NO)
    await uasyncio.sleep(1)  # wait for 1 second
    queue.put_sync(VoteCommand.INDICATE_YES)
    await uasyncio.sleep(1)  # wait for 1 second
    queue.put_sync(VoteCommand.STOP)
